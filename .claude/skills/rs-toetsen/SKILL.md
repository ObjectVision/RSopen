...
## Landbouw: een claimband van nul legt de arealen vast, niet de plek

`ModelParameters/Landbouw/claims_conservatief/Claimbandbreedte` staat standaard op 0 en is via de omgevingsvariabele `RSO_CLAIMBANDBREEDTE` te zetten. Bij 0 zijn onder- en bovengrens per landbouwklasse per allocatieregio allebei gelijk aan het basisjaarareaal maal de restclaimfactor. Dat legt het areaal per klasse en per regio vast, en daarmee de claimrealisatie, maar niet de plek. De vier akkerbouwklassen Aardappelen, Bieten, Granen en Akkerbouw delen hun labels in de transitiekostenmatrix, dus een wissel tussen die vier kost niets (#202), en `discrete_alloc_sp` zet ze neer waar hun geschiktheid het hoogst is. Gemeten op zichtjaar 2040 bij #837: van die vier klassen staat 15 tot 35 procent van het basisjaarareaal op een cel die in het basisjaar dezelfde klasse droeg, en over alle klassen houdt 71 tot 76 procent van de gealloceerde cellen de basisjaarklasse. Klassen met een eigen systeem, zoals grasland, boomgaard, bollen en boomkwekerij, wisselen alleen waar het verschil in contante waarde de transitiekosten goedmaakt.

Wat een wijziging bij band 0 doet hangt dus af van wat zij raakt. Een wijziging aan de transitiekosten die alleen de niet-nul paren raakt werkt niet door: bij #202 was `Alloc_Result` na een volledige herbouw van de matrix byte-identiek. Een wijziging aan de geschiktheid of het saldo werkt wel door, in de plek en niet in de arealen: de herstelde dervingskaarten van #837 verschoven bij band 0 op zichtjaar 2040 421.837 tot 430.690 ha van klasse, bij gelijke arealen per klasse en provincie. Toets een wijziging aan de landbouweconomie daarom op drie dingen:
...
2. De plek bij band 0: een kruistabel van de gealloceerde klasse voor en na, en het aandeel cellen dat de basisjaarklasse houdt.
3. De arealen met `RSO_CLAIMBANDBREEDTE` op 0,05 en op 0,25, de bandbreedtetoets. Blijft het totaal gelijk en gaat de verschuiving tussen de gewassen, dan draagt de economie de verdeling; verdwijnt areaal naar de restcategorie NonLandbouw, dan heeft daar geen gewas een positieve contante waarde.

Een landbouw-alleen-allocatie van een zichtjaar kost twee tot drie minuten en piekt op 35 GB, met `ModelParameters/OntkoppelStedelijkeKlasses` op TRUE in een scratchkopie met een eigen LocalData waarin de standtifs van dat zichtjaar staan. Alleen het eerste zichtjaar is zo exact: in latere zichtjaren leest de allocatie de herkomst voor de transitiekosten uit de stand van hetzelfde zichtjaar op schijf, en niet uit de nieuwe uitkomst van het vorige.
...
Leg de oordelen per wijziging vast in het issue van de toetsronde, met de codestand (commit of tag) waarvoor ze gelden. Een werklijst als hulpmiddel tijdens de ronde blijft in de werkkopie en gaat niet in git: hij hoort bij een projecttak en bij een moment, en via de merges belandt hij anders op elke tak. Vier oordelen: geslaagd, gezakt, zwak (een getal zonder referentie) en open (geen meetpunt). Open betekent zelden tijdgebrek en meestal dat er geen meetpunt bestaat; een nieuw meetpunt in het diagnoseharnas kost ongeveer een kwartier schrijven en een minuut rekenen en maakt de wijziging voorgoed meetbaar. Zet bovenaan op welke codestand de oordelen gelden.
