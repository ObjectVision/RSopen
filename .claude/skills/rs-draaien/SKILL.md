...
### De tabellen uit de geschreven export

De tweede helft van #824 (#831): de export van het exportzichtjaar gaat in twee processen, eerst `Export/generates/Generate_Kaarten` met `ModelParameters/TabellenUitExport` op FALSE, daarna `Themas/Tabellen` en per volgende indeling `Indicatoren_PerIndeling` met de schakelaar op TRUE. Elk indicatorsjabloon heeft daarvoor drie containers: `Lees` (StorageReadOnly-lezers op dezelfde bestandsnamen als de schrijvers), `Live` (de levende items) en `Bron`, dat op de schakelaar naar een van beide wijst; de aggregaties `Per_Regio` en `Per_NL` lezen `Bron`. Een tabel kost zo minuten in plaats van drie kwartier, en een gerepareerde kolom geen volledige export. `RunIndicatoren.ps1` doet dit standaard en zet de omgevingsvariabele per stap; `-TabellenUitExport:$false` geeft de levende route. Bewezen op `Landschap_Veen`, alle veertien tabellen gelijk tot op het teken, zie #824.

Twee grenzen. De schakelaar mag nooit aan staan in het proces dat de kaarten schrijft: een lezer kent zijn schrijver niet, dus in een proces kan een tabel een tif openen voordat hij geschreven is. En een tabelstap eist dat elke teruggelezen kaart op schijf staat; een export van voor #831 mist de ruim dertig kaarten die sindsdien worden geschreven, dus een tabel op zo'n oudere export vraagt `-TabellenUitExport:$false`. De bereikbaarheid van groen is de uitzondering in de manier van rekenen: per regio leest ze alleen haar invoer terug en rekent de potentialen opnieuw, omdat een regio woningen en groen op haar eigen grens afsnijdt; alleen de landelijke tabel leest de groenkaarten zelf terug. De twee GeoDMS-valkuilen die het bewijs vond, een StorageName op een item met subitems en een tif zonder interne georeferentie in een map van meer dan 1000 bestanden, staan in de skill geodms-valkuilen.
