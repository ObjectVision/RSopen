...
Twee valkuilen bij het inkorten. `Classifications/Modellering/StandVar_Prep` hangt af van `SectorAllocRegio/Uq_Sectors/HasWerkenSector`, dus zonder werken verdwijnen de banen-standvariabelen en breekt alles wat daarop leunt. En de bestandsnaam van de standtifs bevat `SS-<aantal xSubsectors>`. Dat getal telt de subsectoren van de ACTIEVE sectoren: `Classifications/Actor/Sector` wordt opgebouwd uit `ModelParameters/SectorAllocRegio/Uq_Sectors/Sectorname`, en `xSubsector` is de union over die sectoren. Commentarieer je een AllocRegio-regel uit terwijl de sector zelf via een andere regel actief blijft, bijvoorbeeld Wonen op COROP terwijl Wonen op NVM blijft staan, dan verandert `Uq_Sectors` niet en blijft de naam `SS-11`, zodat bestaande tifs vindbaar blijven. Zet je een hele sector aan of uit, dan schuift het getal wel en zijn de bestaande tifs onbereikbaar.

Gemeten op 2026-09-08: de sector Landbouw aanzetten in `SectorAllocRegio` brengt er vijftien subsectoren bij (`Classifications/Actor/LandbouwKlasses`, NrOfRows = 15) en de naam gaat van `SS-11` naar `SS-26`. Het log zoekt dan `.../StandY2120/Waterberging/Totaal_Nederland_SS-26.tif` terwijl er `SS-11` op schijf staat. Elke bestaande allocatie is daarmee in een klap onbruikbaar, ook voor de indicatorenkant met `StandAllocatieOntkoppeld` op TRUE. Een sector aan- of uitzetten is dus geen instelling die je vlak voor een levering nog even meeneemt: het is een volledige herberekening van alle varianten en alle zichtjaren.
